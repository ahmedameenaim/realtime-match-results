...
const StartedGame = ({game,onGoal,onCard,onGameEvent}) => {
    const homePlayers = [];
    const awayPlayers = [];
    ...
    for(let i = 1 ; i <= 11 ; ++i) {
        ...
        if(game.home.cards[playerId]){
            homeLabel = <Label color = {game.home.cards[playerId]} ribbon>{game.home.cards[playerId]}</Label>;
        } else {
            homeLabel = game.home.players[playerId];
        }
        let awayLabel;
        if(game.away.cards[playerId]) {
            awayLabel = <Label color = {game.away.cards[playerId]} ribbon>{game.away.cards[playerId]}</Label>;
        } else {
