...
import {Segment,Button,Form,Grid,Header} from 'semantic-ui-react';
 ...
 const UnstartedGame = ({game,onTeamUpdated,onPlayerUpdated,onCancel,onStart}) => {
     const homePlayers = [];
     const awayPlayers = [];
     for(let i = 1 ; i <= 11 ; ++i) {
         homePlayers.push(
         <input placeholder = {`Home Player ${i}`}
         value = {game.home.players[`player_${i}` || '']}
         onChange = {(e) => onPlayerUpdated('home',`player${i}`,e.target.value)}
         key = {`home.players.player_${i}`}/>);

         awayPlayers.push(
             <input placeholder = {`Away Player ${i}`}
             value = {game.away.players[`player_${i}` || '']}
             onChange = {(e) => onPlayerUpdated('away',`player${i}`,e.target.value)}
                 ...
                 <Grid>
                     ...
                     <Grid.Row columns = {1}>
                         <Grid.Column textAlign = "right">
                             <Button.Group>
                                 <Button primary onClick = {onStart}>Start</Button>
                                 <Button.Or />
